    ...
    <Card className="border-teal/30">
      <CardContent className="p-4 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm font-semibold text-foreground">New On-Chain Market</p>
          <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => { setOpen(false); reset(); }}>
        ...
        <div className="space-y-1.5" ref={searchRef}>
          ...
          {selected && (
            <div className="flex items-center gap-2 rounded-md bg-teal/10 border border-teal/30 px-2.5 py-1.5">
              ...
              <span className="text-xs text-muted-foreground ml-auto shrink-0">ID {selected.termId}</span>
          ...
          {showDropdown && !selected && debouncedSearch.length >= 2 && (
            <div className="absolute z-50 left-0 right-0 mt-1 rounded-lg border border-border bg-background shadow-lg max-h-64 overflow-y-auto">
                ...
                <>
                  ...
                  {atoms.map((a) => (
                    <button key={a.term_id} onMouseDown={() => pickAtom(a)}
                      ...
                      <span className="text-sm text-foreground truncate">{a.label ?? `Atom #${a.term_id}`}</span>
                      <span className="ml-auto text-xs text-muted-foreground shrink-0">#{a.term_id}</span>
                    </button>
                  ))}
                </>
              )}
                ...
                <>
                  ...
                  {triples.map((t) => (
                    <button key={t.term_id} onMouseDown={() => pickTriple(t)}
                      ...
                      <span className="text-sm text-foreground truncate">{tripleLabel(t)}</span>
                      <span className="ml-auto text-xs text-muted-foreground shrink-0">#{t.term_id}</span>
                    </button>
                  ))}
                </>
              )}
